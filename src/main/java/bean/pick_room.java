...
public class pick_room {
	...
	public boolean addPick() throws SQLException, ParseException{
		...
		try{
			...
			String sql = "insert into `pick_room` (customer_idcard,room_id,customer_name,pick_time,return_time,room_model,plan_rent,sub_id,pick_status) values(?,?,?,?,?,?,?,?,?)";
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setString(1, this.customer_idcard);
			ps.setString(2, this.room_id);
			ps.setString(3, this.customer_name);
			ps.setDate(4, this.pick_time);
			ps.setDate(5, this.return_time);
	...
	public static List<pick_room> getPicks() throws SQLException{
		Connection conn = null;
		ResultSet rs = null;
		List<pick_room> allpicks = new ArrayList<pick_room>();
		try{
			conn = DBConnection.getConnection();
			...
			rs = ps.executeQuery();
			while(rs.next()){
				pick_room temp = new pick_room(rs.getInt("pick_id"),rs.getString("room_id"),rs.getString("customer_idcard"),rs.getString("customer_name"),rs.getDate("pick_time"),rs.getDate("return_time"),rs.getString("room_model"),rs.getInt("plan_rent"),rs.getInt("sub_id"),rs.getString("pick_status"));
				allpicks.add(temp);
			}
			//System.out.println(rs.next());
		}finally{
